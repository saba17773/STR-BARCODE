<?php $this->layout("layouts/handheldmobile", ['title' => 'Send WMS']); ?>
...
<div>
	<div style="text-align: center; padding: 10px; font-weight: bold;">Send To WMS</div>
	<hr />
	<div style="text-align: center;">
		<label>Barcode</label> <br> <br>
		<input type="text" name="check_barcode" id="check_barcode" class="inputs" autocomplete="off">
	</div>
</div>
...
<script>
	jQuery(document).ready(function($) {
		...
		$('#check_barcode').keydown(function(e) {
            
			if (e.which === 13) {
                
				gojax('post', '/api/v1/wms/check_save', {
					barcode: $('#check_barcode').val()
				}).done(function(data) {
					if (data.result === true) {

<?php $this->layout("layouts/base", ['title' => 'Create New Return Journal']); ?>
...
<div class="row">
  <div class style="margin: 0 auto; max-width: 600;">

    <h3 class="head-text">Create New Return Journal</h3>
    ...
    <div class="panel panel-default hide" id="panel-barcode">
      <div class="panel-body">
        <form id="formBarcode">
          <div class="form-group">
            <label for="journalId">ReturnJournal ID</label>
            <input type="text"  id="journalId" name="journalId" class="form-control input-lg" readonly>
          </div>

          <div class="form-group">
            <label for="requsition">Return Cause</label>
...
jQuery(document).ready(function($) {
  ...
  if (!!qs('j')) {
    ...
    // }, 1000);
    var reId = $('input[name=journalId]').val();


    gojax('get', base_url + "/api/returncause_note/all/"+reId)
      .done(function(data) {
  ...
  $('#completeJournal').on('click', function(event) {
    ...
    $( "#dialog-confirm" ).dialog({
      ...
      buttons: {
        "Yes": function() {
          gojax('post', base_url+'/api/returncomplete_issue/complete', {
            journalId: $('#journalId').val()
          })
          .done(function(data) {
            if (data.status == 200) {
              window.location = base_url+'/movement/issue_returncause';
            } else {
  ...
  $('#modal_alert').on('click', function(event) {
	...
	$('#barcode').keydown(function(event) {
      ...
      if (!!$('#requsition_value').val()) {
        ...
        gojax_f('post', base_url+'/api/ReturnCause/issue/save', '#formBarcode')
        .done(function(data) {
          if (data.status != 200) {
            $('#modal_alert').modal({backdrop: 'static'});
            $('#modal_alert_message').text(data.message);
    ...
    $( "#dialog-confirm" ).dialog({
      ...
      buttons: {
        "Yes": function() {
          ...
          gojax_f('post', base_url+'/api/journal/table/save1', '#formNewIssue')
            .done(function(data) {
              if (data.status === 200) {
                ...
                $('#modal_alert_message').text(data.message);
                $('#top_alert').hide();
                $('form#formNewIssue').trigger('reset');
              }
            });
